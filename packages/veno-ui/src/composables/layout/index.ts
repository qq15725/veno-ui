...
export const VenouiLayoutKey: InjectionKey<LayoutProvide> = Symbol.for('veno-ui:layout')
...
export function useMain () {
  const layout = inject(VenouiLayoutKey)

  if (!layout) throw new Error('Could not find injected veno layout')
...
const generateLayers = (
  ...
  const ids = !layout.length
    ? registered
    : layout.map(l => l.split(':')[0]).filter(l => registered.includes(l))
  ...
  const computedOverlaps = computed(() => {
    ...
    for (const overlap of overlaps.filter(item => item.includes(':'))) {
      ...
      map.set(bottom, {
        ...
        amount: parseInt(topAmount.value as string, 10)
      })

      map.set(top, {
        position: bottomPosition.value,
        amount: -parseInt(bottomAmount.value as string, 10)
      })
    }
