...
export function usePositionStrategies (
  ...
  watchEffect(async () => {
    ...
    scope.run(() => {
      ...
      updatePosition.value = positionStrategy(
        data,
        props,
        contentStyles,
        anchorClasses
...
function connectedPositionStrategy (
  data: PositionStrategyData,
  props: StrategyProps,
  contentStyles: Ref<Dictionary<string>>,
  anchorClasses: Ref<string[]>
  ...
  function updatePosition () {
    ...
    const anchor = fitsY
      ...
      || (preferredAnchor.value.side === 'top' && freeSpace.bottom > freeSpace.top)
    ...
    const canFill = doesOverlap.value
      || ['center', 'top', 'bottom'].includes(anchor.side)

    const maxWidth = canFill
      ? Math.min(viewportWidth, Math.max(targetBox.width, viewportWidth - viewportMargin * 2))
      : anchor.side === 'end'
        ? freeSpace.right
        : anchor.side === 'start'
          ? freeSpace.left
